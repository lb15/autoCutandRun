...
while IFS=, read project sample R1 R2 control;do
	...
	if [ "$control" != "none" ]; then
		...
		qsub -N macs2_"$project"_"$sample" $script_dir/macs2_nodups_onlycontrol.sh $project $sample $tmp $control
		qsub -hold_jid macs2_"$project"_"$sample" -N chpskr_"$project"_"$sample"_"$control" $script_dir/chpskr_general.sh $tmp/$project/"$sample"_vs_"$control"/MACS2/"$sample"_vs_"$control"_nodups_peaks.narrowPeak
	else
	>&2 echo 'No control sample indicated'
	fi
done < $file

...
tmp=/wynton/group/reiter/lauren/Gli2_CR
...
while IFS=, read project sample R1 R2 control;do
	...
	if [ "$control" != "none" ]; then
		...
		qsub -N seacr_"$project"_"$sample" $script_dir/seacr.sh $project $sample $tmp $control
	else
		>&2 echo "Running peak calling without control samples"
		qsub -N seacr_"$project"_"$sample" $script_dir/seacr.sh $project $sample $tmp 
	fi
done < $file
